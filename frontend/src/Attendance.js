...
function Attendance() {
  ...
  const [employeeId, setEmployeeId] = useState(""); // For marking attendance
  const [viewEmployeeId, setViewEmployeeId] = useState(""); // For viewing attendance
  ...
  const markAttendance = async () => {
    ...
    try {
      ...
      fetchAttendance(); // optional: refresh view
    } catch (err) {
  ...
  const fetchAttendance = async () => {
    ...
    setLoading(true);
    let url = `http://localhost:8000/api/attendance/${viewEmployeeId}`;
